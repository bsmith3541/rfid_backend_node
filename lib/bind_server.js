...
BindServer.defaultHost = 'http://pacific-chamber-2346.herokuapp.com/';
//BindServer.defaultPort = 8888;
BindServer.defaultPort = 3000;

BindServer.prototype = {

  // Get the proper status code for the /_status endpoint
  getStatusCode: function (callback) {
    fs.readFile('code', 'utf8', function (err, data) {
      callback(err ? 500 : parseInt(data, 10));
    });
  },
  ...
  handleRequest: function (request, response) {
    if (request.url === '/_status') {
      this.getStatusCode(function (code) {
